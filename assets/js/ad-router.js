export class AdRouter {
  ...
  select(context=[]){const tags=new Set(context);const eligible=this.eligible();const intents=['camera','smart-lock','sensor-light','alarm','home-security'];const activeIntent=context.find(t=>intents.includes(t));const pool=activeIntent&&eligible.some(ad=>(ad.category||[]).includes(activeIntent))?eligible.filter(ad=>(ad.category||[]).includes(activeIntent)):eligible;return pool.map(ad=>({...ad,score:Number(ad.priority||0)+(ad.category||[]).reduce((n,t)=>n+(tags.has(t)?20:0),0)})).sort((a,b)=>b.score-a.score)[0]||null}
...
'away':['留守','長期不在','外出'],
'door':['玄関','ドア'],'outdoor':['屋外','ベランダ','駐車場'],'alarm':['防犯ブザー','防犯アラーム','ブザー']};
...
export async function mountAffiliateSlots(){const slots=[...document.querySelectorAll('[data-affiliate-slot]')];if(!slots.length)return;try{const res=await fetch('data/ads.json',{cache:'no-store'});if(!res.ok)throw new Error(`ad registry ${res.status}`);const base=await res.json();let runtime={ads:[]};try{const rr=await fetch('data/ads.runtime.json',{cache:'no-store'});if(rr.ok)runtime=await rr.json()}catch{}const map=new Map((base.ads||[]).map(ad=>[ad.id,ad]));(runtime.ads||[]).forEach(ad=>map.set(ad.id,{...(map.get(ad.id)||{}),...ad}));const router=new AdRouter({ads:[...map.values()]});const inferred=inferPageContext();slots.forEach(slot=>{const explicit=(slot.dataset.context||'').split(',').map(v=>v.trim()).filter(Boolean);router.render(slot,[...new Set([...explicit,...inferred])])})}catch(e){console.warn('[affiliate] registry unavailable',e);slots.forEach(s=>s.hidden=true)}}
